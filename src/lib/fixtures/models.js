export default [
    {
        type:           'open-ai',
        id:             'gpt-5-nano',
        name:           'GPT-5 nano',
        short_name:     'GPT-5 nano',
        icon:           'gpt-5-nano.png',
        family:         'Open AI',
        hosted_at:      'openai.com',
        context_window: 400000,
        is_reasoner:    true,
        pricing_id:     'gpt-5-nano',
        settings:       ['verbosity', 'reasoning_effort'],
        tools:          ['web_search']
    },
    {
        type:           'open-ai',
        id:             'gpt-5-mini',
        name:           'GPT-5 mini',
        short_name:     'GPT-5 mini',
        icon:           'gpt-5-mini.png',
        family:         'Open AI',
        hosted_at:      'openai.com',
        context_window: 400000,
        is_reasoner:    true,
        pricing_id:     'gpt-5-mini',
        settings:       ['verbosity', 'reasoning_effort'],
        tools:          ['web_search']
    },
    {
        type:           'open-ai',
        id:             'gpt-5.2',
        name:           'GPT-5.2',
        short_name:     'GPT-5.2',
        icon:           'gpt-5.png',
        family:         'Open AI',
        hosted_at:      'openai.com',
        context_window: 400000,
        is_reasoner:    true,
        pricing_id:     'gpt-5.2',
        settings:       ['verbosity', 'reasoning_effort'],
        tools:          ['web_search']
    },
    {
        type:           'open-ai',
        id:             'o3',
        name:           'o3',
        short_name:     'o3',
        icon:           'o3.png',
        family:         'Open AI',
        hosted_at:      'openai.com',
        context_window: 200000,
        is_reasoner:    true,
        pricing_id:     'o3',
        settings:       ['reasoning_effort'],
        tools:          ['web_search']
    },
    {
        type:           'open-ai',
        id:             'o3-pro',
        name:           'o3 pro',
        short_name:     'o3 pro',
        icon:           'o3-pro.png',
        family:         'Open AI',
        hosted_at:      'openai.com',
        context_window: 200000,
        is_reasoner:    true,
        pricing_id:     'o3-pro',
        settings:       ['reasoning_effort'],
        tools:          ['web_search']
    },
    {
        type:           'anthropic',
        id:             'claude-haiku-4-5',
        name:           'Claude Haiku 4.5',
        short_name:     'Claude',
        icon:           'claude-haiku.png',
        family:         'Anthropic',
        hosted_at:      'anthropic.com',
        context_window: 200000,
        is_reasoner:    true,
        pricing_id:     'claude-haiku',
        settings:       ['temperature'],
        tools:          ['thinking_budget', 'web_search', 'exa_search']
    },
    {
        type:           'anthropic',
        id:             'claude-sonnet-4-5',
        name:           'Claude Sonnet 4.5',
        short_name:     'Claude',
        icon:           'claude-sonnet.png',
        family:         'Anthropic',
        hosted_at:      'anthropic.com',
        context_window: 200000,
        is_reasoner:    true,
        pricing_id:     'claude-sonnet',
        settings:       ['temperature'],
        tools:          ['thinking_budget', 'web_search', 'exa_search']
    },
    {
        type:           'anthropic',
        id:             'claude-opus-4-5',
        name:           'Claude Opus 4.5',
        short_name:     'Claude',
        icon:           'claude-opus.png',
        family:         'Anthropic',
        hosted_at:      'anthropic.com',
        context_window: 200000,
        is_reasoner:    true,
        pricing_id:     'claude-opus',
        settings:       ['temperature'],
        tools:          ['thinking_budget', 'web_search']
    },
    {
        type:           'google',
        id:             'gemini-2.5-flash-lite',
        name:           'Gemini 2.5 Flash Lite',
        short_name:     'Gemini',
        icon:           'gemini-flash-lite.png',
        family:         'Google',
        hosted_at:      'googleapis.com',
        context_window: 1000000,
        is_reasoner:    false,
        pricing_id:     'gemini-2.5-flash-lite',
        settings:       ['temperature', 'top_p'],
        tools:          ['google_search']
    },
    {
        type:           'google',
        id:             'gemini-3-flash-preview',
        name:           'Gemini 3 Flash',
        short_name:     'Gemini',
        icon:           'gemini-flash.png',
        family:         'Google',
        hosted_at:      'googleapis.com',
        context_window: 1000000,
        is_reasoner:    true,
        pricing_id:     'gemini-3-flash',
        settings:       ['temperature', 'top_p'],
        tools:          ['google_search', 'context_cache']
    },
    {
        type:           'google',
        id:             'gemini-3-pro-preview',
        name:           'Gemini 3 Pro',
        short_name:     'Gemini',
        icon:           'gemini-pro.png',
        family:         'Google',
        hosted_at:      'googleapis.com',
        context_window: 1000000,
        is_reasoner:    true,
        pricing_id:     'gemini-3-pro',
        settings:       ['temperature', 'top_p'],
        tools:          ['google_search', 'context_cache']
    },
    {
        type:           'x',
        id:             'grok-4-1-fast-non-reasoning',
        name:           'Grok 4.1 Fast',
        short_name:     'Grok',
        icon:           'grok-fast.png',
        family:         'X',
        hosted_at:      'x.ai',
        context_window: 2000000,
        is_reasoner:    false,
        pricing_id:     'grok-4-fast',
        settings:       ['temperature', 'top_p'],
        tools:          ['x_search']
    },
    {
        type:           'x',
        id:             'grok-4-1-fast-reasoning',
        name:           'Grok 4.1 Fast (Reasoning)',
        short_name:     'Grok',
        icon:           'grok-fast-reasoning.png',
        family:         'X',
        hosted_at:      'x.ai',
        context_window: 2000000,
        is_reasoner:    true,
        pricing_id:     'grok-4-fast',
        settings:       ['temperature', 'top_p'],
        tools:          ['x_search']
    },
    {
        type:           'x',
        id:             'grok-4',
        name:           'Grok 4',
        short_name:     'Grok',
        icon:           'grok.png',
        family:         'X',
        hosted_at:      'x.ai',
        context_window: 256000,
        is_reasoner:    true,
        pricing_id:     'grok',
        settings:       ['temperature', 'top_p'],
        tools:          ['x_search']
    },
    {
        type:           'openrouter',
        id:             'meta-llama/llama-4-scout',
        name:           'Llama 4 Scout',
        short_name:     'Scout',
        icon:           'llama-light.png',
        family:         'Meta',
        hosted_at:      'openrouter.ai',
        context_window: 10000000,
        is_reasoner:    false,
        pricing_id:     'llama-4-scout',
        settings:       ['temperature', 'top_p'],
        tools:          []
    },
    {
        type:           'openrouter',
        id:             'meta-llama/llama-4-maverick',
        name:           'Llama 4 Maverick',
        short_name:     'Maverick',
        icon:           'llama-medium.png',
        family:         'Meta',
        hosted_at:      'openrouter.ai',
        context_window: 1000000,
        is_reasoner:    false,
        pricing_id:     'llama-4-maverick',
        settings:       ['temperature', 'top_p'],
        tools:          []
    },
    {
        type:           'openrouter',
        id:             'amazon/nova-2-lite-v1',
        name:           'Nova 2 Lite',
        short_name:     'Nova',
        icon:           'nova-lite.png',
        family:         'Amazon',
        hosted_at:      'openrouter.ai',
        context_window: 1000000,
        is_reasoner:    false,
        pricing_id:     'nova-lite',
        settings:       ['temperature', 'top_p'],
        tools:          []
    },
    {
        type:           'openrouter',
        id:             'amazon/nova-pro-v1',
        name:           'Nova Pro 1.0',
        short_name:     'Nova',
        icon:           'nova-pro.png',
        family:         'Amazon',
        hosted_at:      'openrouter.ai',
        context_window: 300000,
        is_reasoner:    false,
        pricing_id:     'nova-pro',
        settings:       ['temperature', 'top_p'],
        tools:          []
    },
    {
        type:           'openrouter',
        id:             'amazon/nova-premier-v1',
        name:           'Nova Premier 1.0',
        short_name:     'Nova',
        icon:           'nova-premier.png',
        family:         'Amazon',
        hosted_at:      'openrouter.ai',
        context_window: 1000000,
        is_reasoner:    false,
        pricing_id:     'nova-premier',
        settings:       ['temperature', 'top_p'],
        tools:          []
    },
    {
        type:           'openrouter',
        id:             'meta-llama/llama-3.1-405b-instruct',
        name:           'Llama 3.1 405b',
        short_name:     'Llama 405b',
        icon:           'llama-heavy.png',
        family:         'Meta',
        hosted_at:      'openrouter.ai',
        context_window: 33000,
        is_reasoner:    false,
        pricing_id:     'llama-3-405b',
        settings:       ['temperature', 'top_p'],
        tools:          []
    },
    {
        type:           'deepseek',
        id:             'deepseek-chat',
        name:           'DeepSeek v3.2 Chat',
        short_name:     'DeepSeek',
        icon:           'deepseek-chat.png',
        family:         'DeepSeek',
        hosted_at:      'deepseek.com',
        context_window: 128000,
        is_reasoner:    false,
        pricing_id:     'deepseek-chat',
        settings:       ['temperature', 'top_p'],
        tools:          []
    },
    {
        type:           'deepseek',
        id:             'deepseek-reasoner',
        name:           'DeepSeek v3.2 Reasoner',
        short_name:     'DeepSeek',
        icon:           'deepseek-reasoner.png',
        family:         'DeepSeek',
        hosted_at:      'deepseek.com',
        context_window: 128000,
        is_reasoner:    true,
        pricing_id:     'deepseek-reasoner',
        settings:       ['temperature', 'top_p'],
        tools:          []
    },
    {
        type:           'openrouter',
        id:             'qwen/qwen-turbo',
        name:           'Qwen3 30b a3b',
        short_name:     'Qwen',
        icon:           'qwen-turbo.png',
        family:         'Qwen',
        hosted_at:      'openrouter.ai',
        context_window: 1000000,
        is_reasoner:    false,
        pricing_id:     'qwen-a3b',
        settings:       ['temperature', 'top_p'],
        tools:          []
    },
    {
        type:           'openrouter',
        id:             'qwen/qwen-plus-2025-07-28',
        name:           'Qwen3 Plus',
        short_name:     'Qwen',
        icon:           'qwen-plus.png',
        family:         'Qwen',
        hosted_at:      'openrouter.ai',
        context_window: 1000000,
        is_reasoner:    false,
        pricing_id:     'qwen-plus',
        settings:       ['temperature', 'top_p'],
        tools:          []
    },
    {
        type:           'openrouter',
        id:             'qwen/qwen3-max',
        name:           'Qwen 3 Max',
        short_name:     'Qwen',
        icon:           'qwen-max.png',
        family:         'Qwen',
        hosted_at:      'openrouter.ai',
        context_window: 1000000,
        is_reasoner:    false,
        pricing_id:     'qwen-max',
        settings:       ['temperature', 'top_p'],
        tools:          []
    },
    {
        type:           'openrouter',
        id:             'moonshotai/kimi-linear-48b-a3b-instruct',
        name:           'Kimi Linear 48b a3b',
        short_name:     'Kimi',
        icon:           'kimi-linear.png',
        family:         'Moonshot',
        hosted_at:      'openrouter.ai',
        context_window: 128000,
        is_reasoner:    true,
        pricing_id:     'kimi-linear-48b',
        settings:       ['temperature', 'top_p'],
        tools:          []
    },
    {
        type:           'openrouter',
        id:             'moonshotai/kimi-k2-0905',
        name:           'Kimi K2',
        short_name:     'Kimi',
        icon:           'kimi-k2.png',
        family:         'Moonshot',
        hosted_at:      'openrouter.ai',
        context_window: 262000,
        is_reasoner:    true,
        pricing_id:     'kimi-k2',
        settings:       ['temperature', 'top_p'],
        tools:          []
    },
    {
        type:           'openrouter',
        id:             'moonshotai/kimi-k2-thinking',
        name:           'Kimi K2 Thinking',
        short_name:     'Kimi',
        icon:           'kimi-k2-thinking.png',
        family:         'Moonshot',
        hosted_at:      'openrouter.ai',
        context_window: 262000,
        is_reasoner:    true,
        pricing_id:     'kimi-k2-thinking',
        settings:       ['temperature', 'top_p'],
        tools:          []
    },
    {
        type:           'mistral',
        id:             'mistral-small-latest',
        name:           'Mistral Small',
        short_name:     'Mistral',
        icon:           'mistral-small.png',
        family:         'Mistral',
        hosted_at:      'mistral.ai',
        context_window: 128000,
        is_reasoner:    false,
        pricing_id:     'free',
        settings:       ['temperature', 'top_p'],
        tools:          []
    },
    {
        type:           'mistral',
        id:             'mistral-medium-latest',
        name:           'Mistral Medium',
        short_name:     'Mistral',
        icon:           'mistral-medium.png',
        family:         'Mistral',
        hosted_at:      'mistral.ai',
        context_window: 128000,
        is_reasoner:    false,
        pricing_id:     'free',
        settings:       ['temperature', 'top_p'],
        tools:          []
    },
    {
        type:           'mistral',
        id:             'mistral-large-latest',
        name:           'Mistral Large',
        short_name:     'Mistral',
        icon:           'mistral-large.png',
        family:         'Mistral',
        hosted_at:      'mistral.ai',
        context_window: 128000,
        is_reasoner:    false,
        pricing_id:     'free',
        settings:       ['temperature', 'top_p'],
        tools:          []
    },
    {
        type:           'groq',
        id:             'openai/gpt-oss-20b',
        name:           'GPT-OSS 20b',
        short_name:     'GPT',
        icon:           'groq-gpt-20b.png',
        family:         'Groq',
        hosted_at:      'groq.com',
        context_window: 128000,
        is_reasoner:    true,
        pricing_id:     'groq-gpt-oss-20b',
        settings:       ['temperature', 'top_p'],
        tools:          []
    },
    {
        type:           'groq',
        id:             'openai/gpt-oss-120b',
        name:           'GPT-OSS 120b',
        short_name:     'GPT',
        icon:           'groq-gpt-120b.png',
        family:         'Groq',
        hosted_at:      'groq.com',
        context_window: 128000,
        is_reasoner:    true,
        pricing_id:     'groq-gpt-oss-120b',
        settings:       ['temperature', 'top_p'],
        tools:          []
    },
    {
        type:           'groq',
        id:             'meta-llama/llama-4-maverick-17b-128e-instruct',
        name:           'Llama 4 Maverick',
        short_name:     'Maverick',
        icon:           'groq-llama.png',
        family:         'Groq',
        hosted_at:      'groq.com',
        context_window: 128000,
        is_reasoner:    false,
        pricing_id:     'groq-llama-4-maverick',
        settings:       ['temperature', 'top_p'],
        tools:          []
    },
    {
        type:           'groq',
        id:             'qwen/qwen3-32b',
        name:           'Qwen3 32b',
        short_name:     'Qwen',
        icon:           'groq-qwen.png',
        family:         'Groq',
        hosted_at:      'groq.com',
        context_window: 128000,
        is_reasoner:    true,
        pricing_id:     'groq-qwen3-32b',
        settings:       ['temperature', 'top_p'],
        tools:          []
    },
    {
        type:           'groq',
        id:             'moonshotai/kimi-k2-instruct-0905',
        name:           'Kimi K2 Instruct',
        short_name:     'Kimi',
        icon:           'groq-moonshot.png',
        family:         'Groq',
        hosted_at:      'groq.com',
        context_window: 128000,
        is_reasoner:    true,
        pricing_id:     'groq-kimi-k2-instruct',
        settings:       ['temperature', 'top_p'],
        tools:          []
    },
    {
        type:           'cohere',
        id:             'command-r',
        name:           'Command R',
        short_name:     'Command R',
        icon:           'command-r.png',
        family:         'Cohere',
        hosted_at:      'cohere.com',
        context_window: 128000,
        is_reasoner:    false,
        pricing_id:     'command-r',
        settings:       ['temperature', 'top_p'],
        tools:          []
    },
    {
        type:           'cohere',
        id:             'command-r-plus-08-2024',
        name:           'Command R+',
        short_name:     'Command R+',
        icon:           'command-r-plus.png',
        family:         'Cohere',
        hosted_at:      'cohere.com',
        context_window: 128000,
        is_reasoner:    false,
        pricing_id:     'command-r-plus',
        settings:       ['temperature', 'top_p'],
        tools:          []
    },
    {
        type:           'openrouter',
        id:             'allenai/olmo-3-7b-instruct',
        name:           'Olmo 3 7b',
        short_name:     'Olmo',
        icon:           'olmo-small.png',
        family:         'AllenAI',
        hosted_at:      'openrouter.ai',
        context_window: 66000,
        is_reasoner:    false,
        pricing_id:     'olmo-3-7b',
        settings:       ['temperature', 'top_p'],
        tools:          []
    },
    {
        type:           'openrouter',
        id:             'allenai/olmo-3-7b-think',
        name:           'Olmo 3 7b Think',
        short_name:     'Olmo',
        icon:           'olmo-medium.png',
        family:         'AllenAI',
        hosted_at:      'openrouter.ai',
        context_window: 66000,
        is_reasoner:    true,
        pricing_id:     'olmo-3-7b-think',
        settings:       ['temperature', 'top_p'],
        tools:          []
    },
    {
        type:           'openrouter',
        id:             'allenai/olmo-3-32b-think',
        name:           'Olmo 3 32b Think',
        short_name:     'Olmo',
        icon:           'olmo-large.png',
        family:         'AllenAI',
        hosted_at:      'openrouter.ai',
        context_window: 66000,
        is_reasoner:    true,
        pricing_id:     'olmo-3-32b-think',
        settings:       ['temperature', 'top_p'],
        tools:          []
    },
    {
        type:           'openrouter',
        id:             'minimax/minimax-01',
        name:           'MiniMax 01',
        short_name:     '01',
        icon:           'minimax-01.png',
        family:         'MiniMax',
        hosted_at:      'openrouter.ai',
        context_window: 4000000,
        is_reasoner:    true,
        pricing_id:     'minimax-01',
        settings:       ['temperature', 'top_p'],
        tools:          []
    },
    {
        type:           'openrouter',
        id:             'minimax/minimax-m2.1',
        name:           'MiniMax M2.1',
        short_name:     'M2.1',
        icon:           'minimax-m-series.png',
        family:         'MiniMax',
        hosted_at:      'openrouter.ai',
        context_window: 205000,
        is_reasoner:    true,
        pricing_id:     'minimax-m2.1',
        settings:       ['temperature', 'top_p'],
        tools:          []
    },
    {
        type:           'inception',
        id:             'mercury',
        name:           'Mercury',
        short_name:     'Mercury',
        icon:           'mercury.png',
        family:         'Inception',
        hosted_at:      'inceptionlabs.ai',
        context_window: 32000,
        is_reasoner:    false,
        is_diffuser:    true,
        pricing_id:     'inception-mercury',
        settings:       ['temperature', 'top_p'],
        tools:          []
    },
    {
        type:           'openrouter',
        id:             'ai21/jamba-mini-1.7',
        name:           'Jamba 1.7 Mini',
        short_name:     'Jamba',
        icon:           'jamba-mini.png',
        family:         'AI21',
        hosted_at:      'openrouter.ai',
        context_window: 256000,
        is_reasoner:    false,
        pricing_id:     'jamba-mini',
        settings:       ['temperature', 'top_p'],
        tools:          []
    },
    {
        type:           'openrouter',
        id:             'ai21/jamba-large-1.7',
        name:           'Jamba 1.7 Large',
        short_name:     'Jamba',
        icon:           'jamba-large.png',
        family:         'AI21',
        hosted_at:      'openrouter.ai',
        context_window: 256000,
        is_reasoner:    false,
        pricing_id:     'jamba-large',
        settings:       ['temperature', 'top_p'],
        tools:          []
    },
    {
        type:           'openrouter',
        id:             'z-ai/glm-4.7',
        name:           'GLM 4.7',
        short_name:     'GLM',
        icon:           'glm-4.png',
        family:         'Z.AI',
        hosted_at:      'openrouter.ai',
        context_window: 128000,
        is_reasoner:    true,
        pricing_id:     'glm-4.6',
        settings:       ['temperature', 'top_p'],
        tools:          []
    },
    {
        type:           'openrouter',
        id:             'openai/gpt-oss-20b',
        name:           'GPT-OSS 20b',
        short_name:     'GPT',
        icon:           'gpt-oss-20b.png',
        family:         'Open AI OSS',
        hosted_at:      'openrouter.ai',
        context_window: 128000,
        is_reasoner:    true,
        pricing_id:     'gpt-oss-20b',
        settings:       ['temperature', 'top_p'],
        tools:          []
    },
    {
        type:           'openrouter',
        id:             'openai/gpt-oss-120b',
        name:           'GPT-OSS 120b',
        short_name:     'GPT',
        icon:           'gpt-oss-120b.png',
        family:         'Open AI OSS',
        hosted_at:      'openrouter.ai',
        context_window: 128000,
        is_reasoner:    true,
        pricing_id:     'gpt-oss-120b',
        settings:       ['temperature', 'top_p'],
        tools:          []
    },
    {
        type:           'openrouter',
        id:             'prime-intellect/intellect-3',
        name:           'Intellect 3',
        short_name:     'Intellect',
        icon:           'intellect.png',
        family:         'Prime Intellect',
        hosted_at:      'openrouter.ai',
        context_window: 128000,
        is_reasoner:    true,
        pricing_id:     'intellect-3',
        settings:       ['temperature', 'top_p'],
        tools:          []
    },
    {
        type:           'openrouter',
        id:             'openai/gpt-oss-20b:free',
        name:           'GPT-OSS 20b',
        short_name:     'GPT',
        icon:           'free-openai.png',
        family:         'Free (OpenRouter)',
        hosted_at:      'openrouter.ai',
        context_window: 128000,
        is_reasoner:    true,
        pricing_id:     'free',
        settings:       ['temperature', 'top_p'],
        tools:          []
    },
    {
        type:           'openrouter',
        id:             'z-ai/glm-4.5-air:free',
        name:           'GLM 4.5 Air',
        short_name:     'GLM',
        icon:           'free-z-ai.png',
        family:         'Free (OpenRouter)',
        hosted_at:      'openrouter.ai',
        context_window: 131000,
        is_reasoner:    true,
        pricing_id:     'free',
        settings:       ['temperature', 'top_p'],
        tools:          []
    },
    {
        type:           'openrouter',
        id:             'nex-agi/deepseek-v3.1-nex-n1:free',
        name:           'DeepSeek v3.1 Nex N1',
        short_name:     'N1',
        icon:           'free-nex.png',
        family:         'Free (OpenRouter)',
        hosted_at:      'openrouter.ai',
        context_window: 131000,
        is_reasoner:    true,
        pricing_id:     'free',
        settings:       ['temperature', 'top_p'],
        tools:          []
    },
    {
        type:           'openrouter',
        id:             'tngtech/deepseek-r1t2-chimera:free',
        name:           'DeepSeek R1T2 Chimera',
        short_name:     'R1T2',
        icon:           'free-tng.png',
        family:         'Free (OpenRouter)',
        hosted_at:      'openrouter.ai',
        context_window: 164000,
        is_reasoner:    true,
        pricing_id:     'free',
        settings:       ['temperature', 'top_p'],
        tools:          []
    },
    {
        type:           'openrouter',
        id:             'nvidia/nemotron-3-nano-30b-a3b:free',
        name:           'Nemotron 3 Nano 30b a3b',
        short_name:     'Nemotron',
        icon:           'free-nvidia.png',
        family:         'Free (OpenRouter)',
        hosted_at:      'openrouter.ai',
        context_window: 256000,
        is_reasoner:    true,
        pricing_id:     'free',
        settings:       ['temperature', 'top_p'],
        tools:          []
    }
]
